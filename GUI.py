...
pygame.display.set_caption('CODE TRANSLATION')
...
# Hàm hỗ trợ
def help():
    global bool_morse_clicked, bool_semaphore_clicked, bool_help_clicked, slider_help_morse, slider_help_semaphore, slider_help_menu
    if bool_morse_clicked:
        slider_help_morse.update(events)
        slider_help_morse.draw(surface_help)
    elif bool_semaphore_clicked:
        slider_help_semaphore.update(events)
        slider_help_semaphore.draw(surface_help)
    else:
        slider_help_menu.update(events)
        slider_help_menu.draw(surface_help)

    surface_screen.blit(surface_help,
                        (surface_screen.get_width() / 2 - surface_help.get_width() / 2,
                         surface_screen.get_height() / 2 - surface_help.get_height() / 2))
...
# Hàm chế độ MORSE
def MORSE():
    global fps, bool_morse_clicked, bool_status_mod, bool_value_error, dic_morse_to_text, dic_text_to_morse, temp_pos1, temp_pos2, text_Morse_surface, text_Document_surface, bool_morse_setting_clicked, text_temp2, text_temp1, string_input, int_check_starter, bool_help_clicked
    ...
    if surface_button_setting.draw(surface_screen):
        effect_clicked()
        bool_morse_setting_clicked = not bool_morse_setting_clicked

    if bool_value_error:
        value_error_gui(txt=True)
    else:
        int_check_starter = 0
    ...
    if surface_button_help.draw(surface_screen):
        effect_clicked()
...
def value_error_gui(txt=False, mp4=False):
    ...
    if txt:
        ...
        surface_screen.blit(text_value_error_txt, (345, 269.5))

        pos_value_error = (175, 100)
        rect_value_error = pygame.rect.Rect(pos_value_error + surface_screen_value_error.get_size())
    else:
        surface_screen_value_error = pygame.Surface((800, 400)).convert_alpha()
        surface_screen_value_error.fill((160, 160, 160))
        surface_screen_value_error.set_alpha(230)
        surface_screen.blit(surface_screen_value_error, (150, 100))

        surface_screen.blit(image_warning, (175, 237.5))

        surface_screen.blit(text_value_error_mp4, (320, 269.5))

        pos_value_error = (150, 100)
        rect_value_error = pygame.rect.Rect(pos_value_error + surface_screen_value_error.get_size())
...
def SEMAPHORE():
    global bool_semaphore_clicked, bool_opencv_screen_clicked, bool_video_active, fps, cap, int_width_cap, int_height_cap, input_width, input_height, bool_value_error, int_check_starter, bool_help_clicked
...
def output(keys, image):
    ...
    keystring = "+".join(keys)
    if keystring:
        to_display = "space" if get_key_text(keys) == " " else "numeral" if get_key_text(
            keys) == "N" else "delete" if get_key_text(keys) == "D" else get_key_text(keys)
...
def OpenCV_screen():
    global bool_opencv_screen_clicked, bool_semaphore_clicked, bool_video_active, fps, cap, last_keys, NUMERAL, counter, int_width_cap, int_height_cap, FLIP, last_image

    int_dt = clock.tick(fps) / 1000.0

    surface_screen.fill((25, 25, 25))
    pygame.draw.rect(surface_screen, (255, 255, 255), (
    (1100 - int_width_cap - 2) / 2, (600 - int_height_cap - 2) / 2, int_width_cap + 2, int_height_cap + 2), 1)

    surface_screen.blit(text_FLIP, (970, 50))
    ...
    if surface_button_flip.draw(surface_screen, int_dt):
        FLIP = not FLIP
        ...
        if success:
            ...
            if pose_results.pose_landmarks:
                ...
                if type_semaphore(armL_angle, armR_angle, NUMERAL):
                    if counter >= 6 and get_key_text(last_keys) != "N":
                        NUMERAL = not NUMERAL
                        counter = 0
            ...
            surface_screen.blit(pygame.surfarray.make_surface(image),
                                ((1100 - int_width_cap) / 2, (600 - int_height_cap) / 2))
            last_image = image
        else:
            bool_video_active = False

    if last_image is not None:
        surface_screen.blit(pygame.surfarray.make_surface(last_image),
                            ((1100 - int_width_cap) / 2, (600 - int_height_cap) / 2))
...
pose_model = mp.solutions.pose.Pose(min_detection_confidence=0.55, min_tracking_confidence=0.5)
...
image_help = pygame.image.load("asset\\Image\\HELP.png").convert_alpha()
...
text_FLIP = pygame.font.Font("asset\\Font\\Saira-Thin.ttf", 30).render("FLIP", True, "#FFFFFF")
...
text_value_error_txt = pygame.font.Font("asset\\Font\\Montserrat-Regular.ttf",
                                        50).render("File format have to .txt",
                                                   True,
                                                   "#000000")

text_value_error_mp4 = pygame.font.Font("asset\\Font\\Montserrat-Regular.ttf",
                                        50).render("File format have to .mp4",
                                                   True,
                                                   "#000000")
...
surface_button_help = Button_IMG(1070, 570, image_help, 0.1, 0.01)

surface_button_quit_help = Button_IMG(950, 120, image_quit, 0.1, 0.02)
...
surface_button_flip = Animation_Toggle((950, 100),
                                       100,
                                       50,
                                       30,
                                       FLIP)
surface_button_flip.set_circle_color((25, 25, 25))
surface_button_flip.set_speed(300)
...
custom_color_renderer = ImSliderRenderer(arrow_color=((255, 255, 255), (68, 68, 68)),
                                         dot_color=((255, 255, 255), (68, 68, 68)),
                                         slide_color=(41, 41, 41),
                                         selection_color=(68, 68, 68),
                                         selection_page_color=(68, 68, 68),
                                         background_color=(41, 41, 41),
                                         border_thickness=4,
                                         radius=0.2)

surface_help = pygame.Surface((900, 450), pygame.SRCALPHA, 32)

slider_help_morse = ImSlider((surface_screen.get_width() / 2 - surface_help.get_width() / 2,
                              surface_screen.get_height() / 2 - surface_help.get_height() / 2),
                             (900, 450), stype=STYPE_FADE, rewind=True, focus=False, renderer=custom_color_renderer)
slider_help_morse.load_images(["asset\\Image\\Help\\2c.png", "asset\\Image\\Help\\2.1c.png"])

slider_help_semaphore = ImSlider((surface_screen.get_width() / 2 - surface_help.get_width() / 2,
                                  surface_screen.get_height() / 2 - surface_help.get_height() / 2),
                                 (900, 450), stype=STYPE_FADE, rewind=True, focus=False, renderer=custom_color_renderer)
slider_help_semaphore.load_images(["asset\\Image\\Help\\3c.png", "asset\\Image\\Help\\3.1c.png"])

slider_help_menu = ImSlider((surface_screen.get_width() / 2 - surface_help.get_width() / 2,
                             surface_screen.get_height() / 2 - surface_help.get_height() / 2),
                            (900, 450), stype=STYPE_FADE, rewind=True, focus=False, renderer=custom_color_renderer)
slider_help_menu.load_images(["asset\\Image\\Help\\1c.png"])
    ...
    while bool_running:
        ...
        if bool_options_clicked:
            ...
            if bool_help_clicked:
                help()
                surface_back_button.disable()
                surface_button_setting.disable()
                surface_button_change.disable()
                surface_button_upload.disable()
                surface_button_speaker.disable()
                surface_button_copy.disable()
            else:
                surface_back_button.enable()
                surface_button_setting.enable()
                surface_button_change.enable()
                surface_button_upload.enable()
                surface_button_speaker.enable()
                surface_button_copy.enable()
        elif bool_semaphore_clicked:
            SEMAPHORE()
            if bool_help_clicked:
                help()
                surface_button_live.disable()
                surface_button_import_file.disable()
            else:
                surface_button_live.enable()
                surface_button_import_file.enable()
