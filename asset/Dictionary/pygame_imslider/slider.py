...
class ImSlider(object):
    ...
    def __init__(self, pos=(0, 0), size=(0, 0), stype=STYPE_SLIDE, per_page=1, per_move=0, focus=True, rewind=False,
        ...
        self.border_thickness = self.renderer.border_thickness
        self.pos_org = pos
        self.dot_rect = pygame.Rect
        self.arrows = (Arrow(self.pos_org, osp.join(HERE, "left.png"), self.renderer, pygame.K_LEFT),
                       Arrow(self.pos_org, osp.join(HERE, "right.png"), self.renderer, pygame.K_RIGHT))
    ...
    def load_images(self, images, lazy=False):
        ...
        self.layout.set_position(self.background.rect.x + self.arrows[0].rect.width + self.border_thickness, self.background.rect.y)
    ...
    def setup_pagination(self):
        ...
        y_margin = 4
        y = self.background.rect.bottom - self.layout.padding + y_margin - self.border_thickness
        ...
        for i in range(max(0, nbr_pages - len(dots))):
            dot = Dot(self.pos_org, osp.join(HERE, "dot.png"), self.renderer)
            self.sprites.add(dot, layer=2)
            dot.set_size(dot_radius, dot_radius)
            dot.set_position(x, y)
            x += dot.rect.width + x_margin

            self.dot_rect = pygame.rect.Rect(x + self.pos_org[0],
                                        y + self.pos_org[1],
                                        *dot.rect.size)
    ...
    def set_size(self, width, height):
        ...
        self.arrows[0].set_position(self.background.rect.x + self.border_thickness,
                                    self.background.rect.centery - self.arrows[0].rect.height // 2)
        # Right Arrow
        self.arrows[1].set_size(arrow_width, 2 * arrow_width)
        self.arrows[1].set_position(self.background.rect.right - self.arrows[1].rect.width - self.border_thickness,
                                    self.background.rect.centery - self.arrows[0].rect.height // 2)
    ...
    def update(self, events):
        ...
        arrow0_rect = pygame.rect.Rect(self.background.rect.x + self.pos_org[0] + self.border_thickness,
                                       self.background.rect.centery - self.arrows[0].rect.height // 2 + self.pos_org[1],
                                       *self.arrows[0].rect.size)

        arrow1_rect = pygame.rect.Rect(self.background.rect.right - self.arrows[1].rect.width + self.pos_org[0] - self.border_thickness,
                                       self.background.rect.centery - self.arrows[0].rect.height // 2 + self.pos_org[1],
                                       *self.arrows[1].rect.size)

        self.arrows[0].set_pos_org(self.pos_org)
        self.arrows[1].set_pos_org(self.pos_org)
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN \
                ...
                if self.arrows[0].visible and arrow0_rect.collidepoint(event.pos):
                    self.on_previous()
                elif self.arrows[1].visible and arrow1_rect.collidepoint(event.pos):
                    self.on_next()

                page = self.get_page_at((event.pos[0] - self.pos_org[0], event.pos[1] - self.pos_org[1]))
                if page is not None:
                ...
                if self.arrows[0].visible and arrow0_rect.collidepoint(finger_pos):
                    self.on_previous()
                elif self.arrows[1].visible and arrow1_rect.collidepoint(finger_pos):
                    self.on_next()

                page = self.get_page_at((finger_pos[0] - self.pos_org[0], finger_pos[1] - self.pos_org[1]))
                if page is not None:
                    self.set_index(self.per_page * page)
    ...
    def update_arrows(self):
        ...
        if len(self.layout.slides) == 1 \
                or (len(self.layout.slides) <= self.per_page <= self.per_move):
            # Only one page
            for i in range(2):
