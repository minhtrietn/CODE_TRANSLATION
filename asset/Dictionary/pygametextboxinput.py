...
class TextInputBox:
    ...
    def __init__(
        ...
        self.start_backspace = 0
        self.delay_backspace = 0
    ...
    def update(self, events, active=True):
        if self.status:
            ...
            if self.check:
                if self.backspace:
                    if time.perf_counter() - self.start_backspace >= 0.5:
                        if time.perf_counter() - self.delay_backspace >= 0.02:
                            self.cursor_visible = True
                            backspace = {'unicode': '\x08', 'key': 8, 'mod': 0, 'scancode': 42, 'window': None}
                            self._process_keydown(backspace)
                            ...
                            self.delay_backspace = time.perf_counter()

                if list(pygame.key.get_pressed())[42] and not self.backspace:
                    self.backspace = True
                    self.start_backspace = time.perf_counter()
                    self.cursor_visible = True
                    backspace = {'unicode': '\x08', 'key': 8, 'mod': 0, 'scancode': 42, 'window': None}
                    self._process_keydown(backspace)
                    ...
                    if self.require_wrap:
                        self.wrap_text(string)
                elif not list(pygame.key.get_pressed())[42]:
                    self.backspace = False
                ...
                for event in events:
                    if event.type == pygame.KEYDOWN and not active:
                        if event.key == pygame.K_c and pygame.key.get_mods() & pygame.KMOD_CTRL:
                            self._process_ctrlc()
                    elif event.type == pygame.TEXTINPUT and active:
                        ...
                        if self.require_wrap:
                            self.wrap_text(string)
                    elif event.type == pygame.KEYDOWN and event.key != pygame.K_BACKSPACE and active:
                        self.cursor_visible = True
